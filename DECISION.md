...
| AD-10 | Frontend dependencies | Mandated libraries + MUI icons + self-hosted fonts; no React Query / form / date / DnD libraries; **amended:** `@mui/x-charts` for the cumulative flow diagram | Fewer dependencies, code the team can explain; zod would duplicate server rules; the chart package follows the MUI mandate and replaces hand-written chart code (user's call, larger bundle accepted) | 2026-09-24 |
