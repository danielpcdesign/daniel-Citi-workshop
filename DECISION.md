...
| AD-17 | Incident state machine | Admin any→any; engineer (assigned only) `Open→In Progress`, `In Progress⇄Blocked`, `In Progress→Resolved`; employee none; only admins close | No skip keeps the engineer-start step in the timeline; unblock avoids admin bottleneck; no review state, so admin closing is the confirmation | 2026-09-23 |
...
| AD-19 | Dashboards and reporting | `reports` service with `/summary` (visibility-scoped), `/hotspots`, `/timings`, `/attention`; SQL aggregates; `status` filter, no window; time to assign (= acknowledged: assignment is an admin's review) + time to resolve, from history; "informed" left to the frontend; no caching | Counting belongs in the database; reassignment makes first-occurrence the honest rule; treating assignment as acknowledgement answers all three of the brief's timing questions | 2026-09-23 |
