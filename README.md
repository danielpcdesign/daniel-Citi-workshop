...
| M3 | Authentication | Registration gated to `acme.inc`, password hashing, JWT issue and verify | In progress — keys generated by Terraform, token verification live in every service (RS256 only, attack cases tested). Endpoints (register, login, refresh, sign-out, `/me`) next |
