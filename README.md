...
| AD-10 | Frontend libraries | **Only what is mandated plus the minimum:** no data-fetching, form, date, or drag-and-drop libraries; `@mui/x-charts` added for the cumulative flow diagram (amended 2026-09-24). The server's validation errors are the form rules, so none are duplicated in JavaScript. |
...
| AD-12 | Errors and validation | **Every error has one shape:** `{"error": {"code", "message", "fields", "request_id"}}`, with a stable `code` the UI branches on and per-field messages shown beside inputs. Beyond the workshop contract's `400/404/500` we also return `401` and `403` (auth), `405` (wrong method) and `409` (duplicates). A `500` never exposes internals — the `request_id` finds the log line instead. Request bodies are validated with Pydantic v2; RFC 9457 Problem Details was considered and judged unnecessary ceremony for one first-party frontend. **Amended 2026-09-24:** shared `_shared/text.py` enforces length limits, rejects control characters (a NUL byte used to reach PostgreSQL and 500), and requires at least one letter in a name; a `psycopg.DataError → 400` safety net catches what a model can't. |
...
| AD-19 | Dashboards | **A read-only `reports` service counts in SQL:** a per-persona summary, location hotspots, timings, and an attention list of blocked and escalated tickets with their reasons. **Timings report time to assign, time to acknowledge, and time to resolve**, reconstructed from the status history and all measured from when the ticket was reported. A ticket counts as acknowledged when its engineer first moves it to `in_progress`. **Amended 2026-09-24 (post-demo):** adds `/flow` (admin) — hourly cumulative-flow snapshots for the CFD, loaded on demand only (AD-14), never polled; see AGENTS.md → AD-19. |
...
- **The top `AppShell` header wrapped at 1280 px once a fifth admin nav link existed.** Adding "Facilities" after "Engineers" (2026-09-24, commit `71c9f23`) pushed the nav past the width that four admin links fit in. Fixed the same commit; verified at 1280 px.
- **A NUL byte in free text crashed the request instead of failing validation.** A NUL
  (`\x00`) in a name, incident title, building name, or a login email reached PostgreSQL,
  which cannot store it in `text`, and came back `500`. Found 2026-09-24 while probing
  whether user input could affect backend data. Fixed: `_shared/text.py` rejects control
  characters before the query runs, and a `psycopg.DataError → 400` safety net in
  `_shared/http.py` catches whatever still gets through (e.g. a NUL in a login-email
  lookup, which has no Pydantic model to reject it first).
- **No length limit on `full_name` or incident `description`.** `full_name` accepted
  100,000 characters and `description` accepted 500,000. Found 2026-09-24, same probe.
  Fixed: `_shared/text.py` enforces per-field maximums (name 100, title 200, reason 1000,
  description/notes 5000), matched by `maxLength` hints and counters on the frontend.
- **A name of only digits was accepted at registration.** Found 2026-09-24 by the user
  while testing. Fixed: `_shared/text.py`'s `person_name()` requires at least one letter
  (the user's call), still allowing digits, spaces, and `'’-.` alongside letters.
...
| `GET` | `/api/reports/flow` | admin | hourly snapshots of how many tickets held each status over the last 90 days, plus a "now" point, replayed from `incident_status_history` via a span join (`lead()`) against `generate_series`; deleted tickets excluded; ~270 kB, ~1 s — loaded on demand only, never polled (added 2026-09-24, post-demo) |
