...
| M13 | Test suites | To the coverage targets in [Testing](#testing) | In progress — backend: 412 tests pass, 100% coverage, 80% gate passes (`.venv/bin/pytest`, 2026-09-24, includes 3 new tests for `GET /api/reports/flow`, commit `cb7dbdd`); mutation spot-check confirms tests fail when the code they cover is broken. Frontend: 214 Vitest/RTL tests pass, 99.47% statements / 95.95% branches, thresholds enforced (`npm run test:coverage`, 2026-09-24, at the clickable-figures / Facilities / CFD commit `71c9f23`; flow utilities tested under two time zones). E2E: Cypress core journey passes against the local stack; in the cloud only smoke checks run (sign-in, deep-link reload, dashboard gate) |
